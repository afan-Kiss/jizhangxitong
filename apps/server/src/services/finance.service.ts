...
export async function createFinanceShareLink(
  ...
  await prisma.financeShareLink.create({
    data: {
      token,
      title: input.title?.trim() || '项目资金报账单',
...
async function mapExpenseRow(
  ...
  if (includeVoucherLinks && attachmentCount) {
    for (const att of expense.attachments) {
      const { token } = createFileAccessTokenWithTtl(creatorId, att.fileId, FILE_TOKEN_TTL_MS)
  ...
  return {
    ...
    reimbursementStatusLabel: reimbursementStatusLabel(expense.reimbursementStatus),
    remark: includeRemarks ? (expense.remark || '') : '',
  }
}
...
export async function getFinanceShareByToken(
  ...
  if (!link) throw new Error('报账链接不存在或已失效')
  if (link.expiresAt && link.expiresAt.getTime() < Date.now()) {
    throw new Error('报账链接已过期')
  }
  ...
  const details = cfg.includeDetails
    ? await Promise.all(
      expenses.map((e) => mapExpenseRow(e, link.createdBy, baseUrl, cfg.includeVoucherLinks, cfg.includeRemarks)),
    )
    : []
  ...
  return {
    ...
    summary: {
      ...
      pendingReimbursementAmount: summary.pendingReimbursementAmount,
...
export async function buildFinanceReportWorkbook(
  ...
  const summarySheet = workbook.addWorksheet('报账汇总')
  summarySheet.addRow(['项目资金报账单', title])
  ...
  summarySheet.addRow(['待报账金额', summary.pendingReimbursementAmount])
  summarySheet.addRow(['已报账金额', summary.reimbursedAmount])
...
export async function exportFinanceExcel(
  ...
  if (params.token) {
    ...
    if (!link) throw new Error('报账链接不存在')
    if (link.expiresAt && link.expiresAt.getTime() < Date.now()) throw new Error('报账链接已过期')
    startDate = link.startDate
    endDate = link.endDate
    title = link.title
  }
