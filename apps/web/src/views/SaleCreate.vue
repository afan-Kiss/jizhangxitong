...
<template>
  <div class="page">
    <van-nav-bar title="销售登记" left-arrow @click-left="router.back()" />
    ...
    <div class="card" v-if="costPreview">
      <h4>成本预览</h4>
      <div>入库成本: ¥{{ costPreview.inboundCost }}</div>
      <div>证书费: ¥{{ costPreview.certificateFee }}</div>
      <div>包装盒: ¥{{ costPreview.packageFee }}</div>
      <div>快递费: ¥{{ costPreview.expressFee }}</div>
      <div>成本调整: ¥{{ costPreview.costAdjustment }}</div>
      <div><strong>销售时总成本: ¥{{ costPreview.totalCost }}</strong></div>
      <div v-if="form.saleAmount">
        <strong>销售毛利: ¥{{ (Number(form.saleAmount) - costPreview.totalCost).toFixed(2) }}</strong>
      </div>
    </div>

    <div style="padding:16px;">
      <van-button type="primary" block round :loading="loading" @click="onSubmit">保存销售</van-button>
    </div>
  </div>
</template>
