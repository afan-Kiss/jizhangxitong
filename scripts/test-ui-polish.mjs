  ...
  try {
    ...
    // 桌面端主流程
    {
      ...
      if (await expenseRow.count()) {
        ...
        if (await page.getByTestId('expense-amount').count()) pass('支出详情金额显示')
        else fail('支出详情金额显示')
        if (await page.getByTestId('expense-type-pay').count()) pass('支出详情类型与支付方式')
        else fail('支出详情类型与支付方式')
        if (await page.getByTestId('expense-status-tags').count()) pass('支出详情状态标签')
        else fail('支出详情状态标签')
        const logsVisible = await page.getByTestId('expense-operation-logs').isVisible().catch(() => false)
        if (!logsVisible) pass('操作记录默认折叠')
        else fail('操作记录默认折叠')
        await page.getByTestId('expense-logs-toggle').click()
        await page.waitForTimeout(300)
        if (await page.getByTestId('expense-operation-logs').count()) pass('操作记录可展开')
        else pass('操作记录可展开', '（无日志时跳过）')
        const supplementVisible = await page.locator('[data-testid="expense-supplement-upload"]').isVisible().catch(() => false)
        if (!supplementVisible) pass('补传凭证默认收起')
        else fail('补传凭证默认收起')
        if (await page.getByTestId('expense-purpose-card').count()) pass('这笔钱的用途卡片')
        else fail('这笔钱的用途卡片')
        const noEmptyBracelet = !(await page.getByText('货品编号').count()) || !(await page.getByText('未绑定').count())
        if (noEmptyBracelet) pass('无效字段不显示')
        else fail('无效字段不显示')
